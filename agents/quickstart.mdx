...
This is the agent-platform-specific quickstart. For the broader Glide bring-up (web app + data layer), see the [main quickstart](/quickstart).

## Prerequisites

- Glide running locally per the [main quickstart](/quickstart) (Postgres on `localhost:5435`, Redis on `localhost:6381`, Inngest dev on `localhost:8288`, web on `localhost:3000`).
...
The MCP gateway speaks MCP spec 2025-11-25. Any MCP-compliant runtime works. Glide ships partner pack drafts for the five hero runtimes — see [Partner registries](/agents/partner-registries).
...
For local dev, generate an HMAC-signed JWT against your dev secret. For production, you'd issue this through your Ory Hydra deployment per the [OAuth flow](/agents/oauth-flow).

### ChatGPT Apps

Submit through the [Partner registry submission flow](/agents/partner-registries) once you've stood up `auth.<your-domain>` + verified `mcp.<your-domain>`.
...
- [OAuth flow](/agents/oauth-flow) — full RFC 7591 + 8707 + PKCE walk-through for production.
- [Tool reference](/agents/tool-reference) — every tool's input + output schema.
- [Self-hosting the agent platform](/agents/self-hosting) — Ory Hydra deploy + production posture.
- [Money-safety contracts](/concepts/money-safety-contracts) — the F-rules every tool path observes.
