...
| **F3** — Fresh-read tenant verification | `@glide/grant-wrapper` re-reads tenant from DB on every tool invocation. Cached grant alone NEVER authorizes. | `@glide/grant-wrapper` |
...
Bearer grants are valid until `exp`. Between issue and use, the principal's tenant membership might be revoked, transferred, or suspended. F3 says: **re-read the tenant row on every tool call, before authorizing.** `@glide/grant-wrapper` is the single point of truth.
...
- [Threat model](/security/threat-model) — STRIDE pass with F-rules as primary mitigations.
- [Agent platform self-host](/agents/self-hosting) — operator-facing F-rule guide.
